...
import './SellerUpdateDress.css';

const SellerUpdateDress = ({ dress, onSuccess, onCancel }) => {
  const { user } = useAuth();
    ...
    if (dress) {
      ...
      if (dress.image) {
        if (dress.image.startsWith('http')) {
          setImageToDisplay(dress.image);
        } else if (dress.image.startsWith('data:image')) {
          setImageToDisplay(dress.image);
        } else {
          setImageToDisplay(`https://localhost:7282/Images/${dress.image}`);
        }
      }
    }
  }, [dress, user]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.isSuccess) {
        ...
        setError(response.error || 'Giyim ürünü güncellenirken bir hata oluştu');
      }
    } catch (error) {
  ...
  return (
    <div className="seller-update-dress">
      <h2>Giyim Ürünü Güncelle</h2>
...
export default SellerUpdateDress; 